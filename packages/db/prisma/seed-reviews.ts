...
function generateRatings(count: number, targetAvg: number): number[] {
  ...
  return Array.from({ length: count }, () => {
    ...
    return Math.max(1, Math.min(5, cumulative.findIndex((c) => rand <= c) + 1));
  });
}
...
async function main() {
  ...
  for (const product of products) {
    ...
    const targetAvg = 3.2 + Math.random() * 1.7;
    ...
    console.log(`  ${product.name}: ${data.length} reviews (target avg ${targetAvg.toFixed(1)})`);
  }

  console.log(`\nDone. Created ${totalCreated} reviews across ${products.length} products.`);
}
